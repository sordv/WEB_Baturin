...
const game = {
  ...
  getRandomFreeCoordinates() {
    ...
    for (let x = 0; x < this.config.getColsCount(); x++) {
      for (let y = 0; y < this.config.getRowsCount(); y++) {
        const point = {x, y};
        if (!this.snake.isOnPoint(point) && !this.food.isOnPoint(point)){
          empty_cells.push(point);
        }
      }
    }

    const Index = Math.floor(Math.random() * empty_cells.length);
  ...
  canSetDirection(direction) {
    ...
    return direction !== inverted_directions[this.direction]
  },
